...
outlier_detection = function(Y, iter = 1) {
  n <- dim(Y)[1];
  ...
  Q3 <- quantile(x, 0.75);
  Q1 <- quantile(x, 0.25);
  upp_whisker <- min(max(x), Q3 + 3*(Q3 - Q1));
  lwr_whisker <- max(min(x), Q1 - 3*(Q3 - Q1));
  index <- which(rowSums(Y) >= upp_whisker | rowSums(Y) <= lwr_whisker);
  outliers <- rownames(Y)[index];
  if (length(index) > 0) {
    type <- rep("type_1", length(index));
  }
  for (i in 1:iter) {
    if (length(index) > 0) {
      Y <- Y[-index,];
    ...
    x <- rowSums(Y);
    y <- rowSums(Y != 0);
    cookd <- cooks.distance(lm(log10(y) ~ x));
    index <- which(cookd >= 4/(length(y) - 1 - 1));
    if (length(index) > 0) {
      outliers <- c(outliers, rownames(Y)[index]);
      type <- c(type, rep("type_2", length(index)));
    } else {
      break;
    }
  }
